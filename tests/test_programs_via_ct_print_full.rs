...
///
/// Column-aware step emission fires one step per distinct `(line, column)`
/// rather than one per line, so a multi-statement line emits several
/// consecutive same-line steps.  These golden vectors predate the
/// column-aware wire shape, so consecutive same-line entries are collapsed
/// to a single observation.
fn observed_step_lines(doc: &serde_json::Value) -> Vec<i64> {
    let mut out: Vec<i64> = Vec::new();
    for (_, l) in step_lines(doc) {
        if out.last() != Some(&l) {
            out.push(l);
        }
    }
...
fn test_control_flow_via_ct_print_full() {
    ...
    // fires one step per distinct `(line, column)` rather than one per
    // line, so this tracks the raw (non-line-deduped) step count: the 115
    // steps the recorder registers plus the entry step `start` emits
    // (trace-events.md, §"The entry step is part of `start`, not the
    // recorder's first `register_step`").
    assert_eq!(counts["values"].as_u64(), Some(116), "values count");
...
fn test_interface_via_ct_print_full() {
    ...
    assert_eq!(
        functions,
        vec!["<toplevel>", "run", "external_call_depth_2"],
...
fn test_create2_via_ct_print_full() {
    ...
    assert_eq!(
        functions,
        vec!["<toplevel>", "run", "external_call_depth_2", "fn_at_0:41"],
        "function table — entry-point + dispatcher orphans + the \
         EXTERNAL CALL placeholder (registered once, reused for both \
