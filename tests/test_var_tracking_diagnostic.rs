...
fn opcode_from_name(name: &str) -> Option<u8> {
    ...
    match name {
        "STOP" => Some(0x00),
        "ADD" => Some(0x01), "MUL" => Some(0x02), "SUB" => Some(0x03),
        "DIV" => Some(0x04), "SDIV" => Some(0x05), "MOD" => Some(0x06),
        "SMOD" => Some(0x07), "ADDMOD" => Some(0x08), "MULMOD" => Some(0x09),
        "EXP" => Some(0x0a), "SIGNEXTEND" => Some(0x0b),
        "LT" => Some(0x10), "GT" => Some(0x11), "SLT" => Some(0x12),
        "SGT" => Some(0x13), "EQ" => Some(0x14), "ISZERO" => Some(0x15),
        "AND" => Some(0x16), "OR" => Some(0x17), "XOR" => Some(0x18),
        "NOT" => Some(0x19), "BYTE" => Some(0x1a),
        "SHL" => Some(0x1b), "SHR" => Some(0x1c), "SAR" => Some(0x1d),
        "SHA3" | "KECCAK256" => Some(0x20),
        "ADDRESS" => Some(0x30), "BALANCE" => Some(0x31),
        "ORIGIN" => Some(0x32), "CALLER" => Some(0x33), "CALLVALUE" => Some(0x34),
        "CALLDATALOAD" => Some(0x35), "CALLDATASIZE" => Some(0x36),
        "CALLDATACOPY" => Some(0x37), "CODESIZE" => Some(0x38),
        "CODECOPY" => Some(0x39), "GASPRICE" => Some(0x3a),
        "EXTCODESIZE" => Some(0x3b), "EXTCODECOPY" => Some(0x3c),
        "RETURNDATASIZE" => Some(0x3d), "RETURNDATACOPY" => Some(0x3e),
        "EXTCODEHASH" => Some(0x3f),
        "BLOCKHASH" => Some(0x40), "COINBASE" => Some(0x41),
        "TIMESTAMP" => Some(0x42), "NUMBER" => Some(0x43),
        "PREVRANDAO" | "DIFFICULTY" => Some(0x44),
        "GASLIMIT" => Some(0x45), "CHAINID" => Some(0x46),
        "SELFBALANCE" => Some(0x47), "BASEFEE" => Some(0x48),
        "BLOBHASH" => Some(0x49), "BLOBBASEFEE" => Some(0x4a),
        "POP" => Some(0x50), "MLOAD" => Some(0x51),
        "MSTORE" => Some(0x52), "MSTORE8" => Some(0x53),
        "SLOAD" => Some(0x54), "SSTORE" => Some(0x55),
        "JUMP" => Some(0x56), "JUMPI" => Some(0x57),
        "PC" => Some(0x58), "MSIZE" => Some(0x59), "GAS" => Some(0x5a),
        "JUMPDEST" => Some(0x5b),
        "TLOAD" => Some(0x5c), "TSTORE" => Some(0x5d), "MCOPY" => Some(0x5e),
        "PUSH0" => Some(0x5f),
        "RETURN" => Some(0xf3), "REVERT" => Some(0xfd),
        "SELFDESTRUCT" => Some(0xff),
        "INVALID" => Some(0xfe),
        "CREATE" => Some(0xf0), "CALL" => Some(0xf1), "CALLCODE" => Some(0xf2),
        "DELEGATECALL" => Some(0xf4), "CREATE2" => Some(0xf5),
...
async fn test_var_tracking_diagnostic() {
    assert!(Command::new("solc").arg("--version").output().map(|o| o.status.success()).unwrap_or(false));
    assert!(Command::new("anvil").arg("--version").output().map(|o| o.status.success()).unwrap_or(false));
    ...
    for f in &ast.functions {
        eprintln!("  {} (src: {}:{} file:{})", f.name, f.src.offset, f.src.length, f.src.file_index);
        for p in &f.parameters {
            eprintln!("    param: {} (decl_off={}, src={}:{})", p.name, p.declaration_offset, p.src.offset, p.src.length);
        }
        for lv in &f.local_variables {
            let stmt = lv.statement_range.as_ref().map(|r| format!("{}:{}", r.offset, r.length)).unwrap_or_else(|| "none".into());
            eprintln!("    local: {} (decl_off={}, src={}:{}, stmt={})", lv.name, lv.declaration_offset, lv.src.offset, lv.src.length, stmt);
        }
    }

    // Get source
    let source_path = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("contracts/FlowTest.sol");
    let source_contents = std::fs::read_to_string(&source_path).unwrap();

    // Get contract
    let contracts = compiled["contracts"].as_object().unwrap();
    let (_, contract_json) = contracts.iter().find(|(k, _)| k.ends_with(":FlowTest")).unwrap();
    ...
    // Deploy and call run()
    let anvil = alloy::node_bindings::Anvil::new().arg("--steps-tracing").spawn();
    ...
    let deploy_receipt = provider.send_transaction(deploy_tx).await.unwrap().get_receipt().await.unwrap();
    ...
    let call_receipt = provider.send_transaction(call_tx).await.unwrap().get_receipt().await.unwrap();
    let tx_hash = call_receipt.transaction_hash;

    // Trace
    let frame = codetracer_evm_recorder::trace_fetcher::fetch_struct_logs(&rpc_url, tx_hash).await.unwrap();
    ...
    for (i, log) in struct_logs.iter().enumerate() {
        ...
        let source_offset = source_map.get_entry_for_pc(pc, &pc_to_idx)
        ...
        if let Some(location) = source_map.resolve_pc(pc, &pc_to_idx, &[source_contents.as_str()]) {
            ...
            if location.line >= 10 && location.line <= 23 {
                ...
                let in_scope: Vec<_> = func.map(|f| f.vars_in_scope_at(source_offset.unwrap_or(-1))).unwrap_or_default();
                ...
                if let Some(op) = opcode {
                    ...
                    eprintln!(
                        ...
                        i, pc, log.op.as_ref(),
                        source_offset.map(|o| o.to_string()).unwrap_or_else(|| "-".into()),
                        stack_depth, tracker.depth(),
                        in_scope_names,
                        assignments.iter().map(|a| a.name.as_str()).collect::<Vec<_>>(),
                        var_vals,
                    );
                }
            } else {
